...
export const httpSchema = z
  .object({
    ...
    url: z
      ...
      .url("A valid http/https url is required."),
    notificationId: z
    ...
    httpAuthMethod: z
      .string({ required_error: "Please select a valid authentication type." })
      .optional(),
  })
...
export const httpSchemaValidation = (
  monitorInfo: IMonitorDocument
): IMonitorErrorMessage => {
  const errors: IMonitorErrorMessage = {};
  const authHeaders = objectValidation(monitorInfo?.headers as string);
  const authBody = objectValidation(monitorInfo?.body as string);
  if (authHeaders) {
